...
| `example_toolgroup.go` | — | action | 按组注册/列举/过滤工具（ToolGroup） |
| `example_context.go` | — | context | 上下文管理（Mode/Ingest/渲染/transient） |
...
go build -o NUL .\example_toolgroup.go && echo "toolgroup: OK"
go build -o NUL .\example_context.go && echo "context: OK"
...
| | 15 | `example_toolgroup.go` | 按组注册/列举/过滤工具（ToolGroup） | 5 min |
| | 16 | `example_context.go` | 上下文管理（Mode/Ingest/渲染/transient） | 5 min |
| **综合** | 17 | `example_server_comprehensive.go` | 完整系统能力串联（server/模型/审计/沙箱） | 15 min |
